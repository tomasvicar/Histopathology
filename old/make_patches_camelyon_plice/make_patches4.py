...
#        lbl=imread_gdal_mask(mask_name,lvl+3)#lvl1 ->+1
        ...
        lbl=imread_gdal_mask(mask_name.replace('\\mask\\','\\mask_dil_lvl3\\'),0)
    else:
...
dataset='cam'
patch_path=r'E:\patches_012s_256_dilatace_add'    
#folder_paths=[r'D:\MPI_CBG\camelyon16\dataset\train\data',r'D:\MPI_CBG\camelyon16\dataset\valid\data',r'D:\MPI_CBG\camelyon16\dataset\test\data']
folder_paths=[r'D:\MPI_CBG\camelyon16\dataset\train\data']
#folder_paths=[r'D:\MPI_CBG\camelyon16\dataset\valid\data']
lvl=0
patch_size=1024#/2 in process
...
for folder_path in folder_paths:
    ...
    for k in range(hotove,kolik,zaras):
        ...
        for kk in range(zaras):
            ...
            imsave(save_path_folder+'data_1_' + str(lbl) + '_' + time_name +'.tif',patch[kk][int(3*patch_size/8):-int(3*patch_size/8),int(3*patch_size/8):-int(3*patch_size/8)],compress=6)

            imsave(save_path_folder+'mask_1_' + str(lbl) + '_' +  time_name +'.tif',mask[kk][int(3*patch_size/8):-int(3*patch_size/8),int(3*patch_size/8):-int(3*patch_size/8)],compress=6)
        
        
            imsave(save_path_folder+'data_2_' + str(lbl) + '_' + time_name +'.tif',patch[kk][int(patch_size/4):-int(patch_size/4):2,int(patch_size/4):-int(patch_size/4):2],compress=6)

            imsave(save_path_folder+'mask_2_' + str(lbl) + '_' +  time_name +'.tif',mask[kk][int(patch_size/4):-int(patch_size/4):2,int(patch_size/4):-int(patch_size/4):2],compress=6)
        

            imsave(save_path_folder+'data_3_' + str(lbl) + '_' + time_name +'.tif',patch[kk][1::4,1::4],compress=6)

            imsave(save_path_folder+'mask_3_' + str(lbl) + '_' +  time_name +'.tif',mask[kk][1::4,1::4],compress=6)
