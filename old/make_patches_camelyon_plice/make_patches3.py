...
dataset='cam'
patch_path=r'D:\MPI_CBG\camelyon16\patches_0s_part2'    
...
for folder_path in folder_paths:
    ...
    for k in range(0,kolik,zaras):
        ...
        for kk in range(zaras):
            ...
            imsave(save_path_folder+'data_1_' + str(lbl) + '_' + time_name +'.tif',patch_s[kk][int(patch_size/4):-int(patch_size/4),int(patch_size/4):-int(patch_size/4)],compress=6)

            imsave(save_path_folder+'mask_1_' + str(lbl) + '_' +  time_name +'.tif',mask_s[kk][int(patch_size/4):-int(patch_size/4),int(patch_size/4):-int(patch_size/4)],compress=6)
        
        
            imsave(save_path_folder+'data_2_' + str(lbl) + '_' + time_name +'.tif',patch_s[kk][::2,::2],compress=6)

            imsave(save_path_folder+'mask_2_' + str(lbl) + '_' +  time_name +'.tif',mask_s[kk][::2,::2],compress=6)
        
        
#            imsave(save_path_folder+'data_3_' + str(lbl) + '_' + time_name +'.tif',patch_l[kk][int(patch_size/4):-int(patch_size/4),int(patch_size/4):-int(patch_size/4)],compress=6)
#
#            imsave(save_path_folder+'mask_3_' + str(lbl) + '_' +  time_name +'.tif',mask_l[kk][int(patch_size/4):-int(patch_size/4),int(patch_size/4):-int(patch_size/4)],compress=6)
        
        
            imsave(save_path_folder+'data_3_' + str(lbl) + '_' + time_name +'.tif',patch_l[kk][::2,::2],compress=6)

            imsave(save_path_folder+'mask_3_' + str(lbl) + '_' +  time_name +'.tif',mask_l[kk][::2,::2],compress=6)
