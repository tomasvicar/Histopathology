...
model_spec='1s_no_pixel011200_0.9635580889522238.pkl'
...
#    file_names = [file_names[i] for i in [51]]
    ...
    #    cn_path=cn_path.replace('pom_plice\\valid\\data','patch_plice\\valid')
        cn_path=cn_path.replace('test_plice/data','patch_plice/test')
    ...
    #        pass
        ...
        patcher=patcher_test.Patcher(img_filename,fg_name,lbl_name,save_name,lvl=lvl,step=step,border=border,write_scale=write_scale)
    ...
    #        img=torch.cat(data, dim=1)
            ...
            img=torch.from_numpy(img).pin_memory().cuda()
