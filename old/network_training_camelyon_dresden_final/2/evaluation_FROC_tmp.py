...
#        Probs, Xcorr, Ycorr = readCSVContent(csvDIR)
        ...
        res_img=imread_gdal_mask_small(r_n,EVALUATION_MASK_LEVEL-2)
...
#        start = time.time()
        tmp =peak_local_max(res_0, min_distance=int(sig), threshold_abs=t)
        Ycorr=list(tmp[:,0])
        Xcorr=list(tmp[:,1])
        Probs=list(res_0[Ycorr,Xcorr])
...
#    plotFROC(total_FPs, total_sensitivity)
    results = griddata(total_FPs, total_sensitivity,[0.25,0.5,1,2,4,8])
    print(results )
...
if __name__ == "__main__":
    ...
    gs=np.linspace(1,30,3)
    ts=np.linspace(0.6,0.95,3)
    sigs=np.linspace(30,300,3)
