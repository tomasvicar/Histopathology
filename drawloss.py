...
class DisplayLosses():
    def __init__(self,evaluate_it=10):
        ...
        self.lr=np.array([],dtype=np.float64)
    ...
    def get_loss_test_last(self):
        return self.loss_test[-1]
    
    def get_data(self):
        return np.stack([self.lr,self.X_train,self.loss,self.acc,self.dice,self.auc,self.sen,self.spe],axis=1),np.stack([self.X_test,self.loss_test,self.acc_test,self.dice_test,self.auc_test,self.sen_test,self.spe_test],axis=1)
