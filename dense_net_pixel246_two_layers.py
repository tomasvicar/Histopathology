...
class PixelNet(nn.Module):
    def __init__(self,k0=16,k=12,input_size=3,gconv=0,p4m=0,return_features=0):
        ...
        self.t1=Conv2BnRelu(3*K, K,filter_size=1,gconv=gconv,p4m=1)
        ...
        self.c_add1=Conv2BnRelu(K, K,gconv=gconv,p4m=1)

        self.t2=Conv2BnRelu(2*K+K_old, K,filter_size=1,gconv=gconv,p4m=1)
        ...
        self.c3_1_1=Conv2BnRelu(K_old, K,filter_size=1,gconv=gconv,p4m=1)
        self.c3_1_3=Conv2BnRelu(K, K,gconv=gconv,p4m=1)
        
        self.c3_2_1=Conv2BnRelu(K+K_old, K,filter_size=1,gconv=gconv,p4m=1)
        self.c3_2_3=Conv2BnRelu(K, K,gconv=gconv,p4m=1)
        
        self.c_add2=Conv2BnRelu(K, K,gconv=gconv,p4m=1)

        self.t3=Conv2BnRelu(2*K+K_old, K,filter_size=1,gconv=gconv,p4m=1)
        ...
        self.c4_1_1=Conv2BnRelu(K_old, K,filter_size=1,gconv=gconv,p4m=1)
        self.c4_1_3=Conv2BnRelu(K, K,gconv=gconv,p4m=1)
        
        self.c4_2_1=Conv2BnRelu(K_old+K, K,filter_size=1,gconv=gconv,p4m=1)
        self.c4_2_3=Conv2BnRelu(K, K,gconv=gconv,p4m=1)
        

        self.t4=Conv2BnRelu(2*K+K_old, K,filter_size=1,gconv=gconv,p4m=1)
        
        self.c_add3=Conv2BnRelu(K, K,gconv=gconv,p4m=1)
        ...
        self.t5=Conv2BnRelu(2*K+K_old, K,filter_size=1,gconv=gconv,p4m=1)
    ...
    def dense_on(self,dense_down=5):
    ...
    def forward(self, x):
        ...
        x=self.initc(x,dil,dense)#246
        
        if self.dense_down==0:
            dense=1
        
        tmp =self.c1_1_1(x,dil,dense)
        y1=self.c1_1_3(tmp,dil,dense)#244
        tmp=self.c1_2_1(self.cat_remove_border((x,y1),dil),dil,dense)
        y2=self.c1_2_3(tmp,dil,dense)#242
        tmp=self.t1(self.cat_remove_border((x,y1,y2),dil))
        x,dil=maxpool_muj(tmp,3,2,dil,dense, self.gconv)#120
        
        
        if self.dense_down==1:
            dense=1
        
        tmp =self.c2_1_1(x,dil,dense)#118
        y1=self.c2_1_3(tmp,dil,dense)
        tmp=self.c2_2_1(self.cat_remove_border((x,y1),dil),dil,dense)
        y2=self.c2_2_3(tmp,dil,dense)#116
        tmp=self.t2(self.cat_remove_border((x,y1,y2),dil))
        
        tmp=self.c_add1(tmp,dil,dense)#114
    
        x,dil=maxpool_muj(tmp,3,2,dil,dense, self.gconv)#56
        ...
        tmp =self.c3_1_1(x,dil,dense)#54
        y1=self.c3_1_3(tmp,dil,dense)
        tmp=self.c3_2_1(self.cat_remove_border((x,y1),dil),dil,dense)
        y2=self.c3_2_3(tmp,dil,dense)#52
        tmp=self.t3(self.cat_remove_border((x,y1,y2),dil))
        
        tmp=self.c_add2(tmp,dil,dense)#114
        ...
        y1=self.c4_1_3(tmp,dil,dense)#22
        tmp=self.c4_2_1(self.cat_remove_border((x,y1),dil),dil,dense)
        y2=self.c4_2_3(tmp,dil,dense)#20
        tmp=self.t4(self.cat_remove_border((x,y1,y2),dil))
        
        tmp=self.c_add3(tmp,dil,dense)#18
        
        x,dil=maxpool_muj(tmp,3,2,dil,dense, self.gconv)#8
        
        
        
        if self.dense_down==4:
            dense=1
        
        tmp =self.c5_1_1(x,dil,dense)
        y1=self.c5_1_3(tmp,dil,dense)#6
        tmp=self.c5_2_1(self.cat_remove_border((x,y1),dil),dil,dense)
        y2=self.c5_2_3(tmp,dil,dense)#4
        tmp=self.t5(self.cat_remove_border((x,y1,y2),dil))
        x,dil=maxpool_muj(tmp,3,2,dil,dense, self.gconv)#1
