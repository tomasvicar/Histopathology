...
#        border_s=int(border*2/fg_scale_down)
        ...
        n_half_s=int(self.n_half/fg_scale_down+1)
        
        corners=[]
        cx= n_half_s
        while cx<self.img_width-step_s-fg_scale_down-1- n_half_s: 
            cy= n_half_s
            while cy<self.img_height-step_s-fg_scale_down-1- n_half_s:
...
#        self.corners=np.stack((tmpx[use],tmpy[use]),axis=1)
    ...
    def read(self):
        ...
        tmp_shape= self.rOverview.ReadAsArray(int(pozicex- self.n_half), int(pozicey-self.n_half), int(2*n), int(2*n))
        patch[:,:,3] =tmp_shape[::2,::2]
        tmp_shape = self.gOverview.ReadAsArray(int(pozicex- self.n_half), int(pozicey- self.n_half), int(2*n), int(2*n))
        patch[:,:,4] =tmp_shape[::2,::2]
        tmp_shape = self.bOverview.ReadAsArray(int(pozicex- self.n_half), int(pozicey- self.n_half), int(2*n), int(2*n))
        patch[:,:,5] =tmp_shape[::2,::2]
