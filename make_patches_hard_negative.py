...
#        lbl=imread_gdal_mask(mask_name.replace('\\mask\\','\\mask_dil_lvl2_16\\'),0)
    else:
...
#        
    ...
    if tumor:
        ...
        fg=fg[:np.min([np.shape(tmp2)[0],np.shape(tmp)[0]]),:np.min([np.shape(tmp2)[1],np.shape(tmp)[1]])]
    
        
    if dataset=='plice':
        lbl_tmp=lbl
        lbl_tmp=lbl_tmp[:np.min([np.shape(tmp2)[0],np.shape(tmp)[0]]),:np.min([np.shape(tmp2)[1],np.shape(tmp)[1]])]
        tmp[lbl_tmp==1]=0
...
dataset='plice'
patch_path=r'E:\histolky_drazdany_data\lungs\patches_nodil_hnm'    
#folder_paths=[r'E:\histolky_drazdany_data\lungs\dataset\train\data',r'E:\histolky_drazdany_data\lungs\dataset\valid\data']
folder_paths=[r'E:\histolky_drazdany_data\lungs\dataset\train\data']
lvl=0

#dataset='cam'
#patch_path=r'D:\MPI_CBG\camelyon16\patches_012s_256'    
...
for folder_path in folder_paths:
    ...
    for k in range(hotovo,kolik,zaras):
        ...
        for kk in range(zaras):
            ...
            imsave(save_path_folder+'mask_1_' + str(lbl) + '_' +  time_name +'.tif',255*mask[kk][int(patch_size/4):-int(patch_size/4),int(patch_size/4):-int(patch_size/4)],compress=6)
        
        
            imsave(save_path_folder+'data_2_' + str(lbl) + '_' + time_name +'.tif',patch[kk][::2,::2],compress=6)

            imsave(save_path_folder+'mask_2_' + str(lbl) + '_' +  time_name +'.tif',255*mask[kk][::2,::2],compress=6)
